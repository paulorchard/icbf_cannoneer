...
public class ExplosionPacket {
    ...
    public static void handle(ExplosionPacket pkt, Supplier<NetworkEvent.Context> ctxSupplier) {
        ...
        ctx.enqueueWork(() -> {
            ...
            IslandCraftMod.LOGGER.info("ExplosionPacket received from {} for pos={} strength={} fire={} mode={}", sender.getName().getString(), pkt.pos, pkt.strength, pkt.fire, pkt.modeOrdinal);
            try {
                // Fallback approach: run a server command to summon a primed TNT with zero fuse.
                ...
                if (sender.getServer() != null) {
                    String cmd = String.format("summon tnt %d %d %d {Fuse:0}", pkt.pos.getX(), pkt.pos.getY(), pkt.pos.getZ());
                    IslandCraftMod.LOGGER.info("Attempting to dispatch server command: {}", cmd);
                    try {
                        Object cmdsObj = sender.getServer().getClass().getMethod("getCommands").invoke(sender.getServer());
                        if (cmdsObj != null) {
                                    ...
                                    if (p1 == String.class) {
                                        try {
                                            if (p0.isAssignableFrom(sender.createCommandSourceStack().getClass())) {
                                                ...
                                                IslandCraftMod.LOGGER.info("Invoked performCommand via reflection (matched {})", method);
                                                invoked = true;
                                                break;
                                            }
                                        } catch (Throwable e) {
                                            IslandCraftMod.LOGGER.warn("performCommand invocation threw: {}", e.toString());
                                        }
                                    }
                                }
                                if (!invoked) {
                                    ...
                                    try {
                                        java.lang.reflect.Method getDisp = cmdsObj.getClass().getMethod("getDispatcher");
                                        Object disp = getDisp.invoke(cmdsObj);
                                        if (disp != null) {
                                            boolean dispInvoked = false;
                                            for (java.lang.reflect.Method method : disp.getClass().getMethods()) {
                                                if (!method.getName().equals("dispatch") || method.getParameterCount() != 2)
                                                    continue;
                                                ...
                                                try {
                                                    Object arg0 = null;
                                                    Object arg1 = null;
                                                    if (a == String.class && b.isAssignableFrom(sender.createCommandSourceStack().getClass())) {
                                                        arg0 = cmd;
                                                        arg1 = sender.createCommandSourceStack();
                                                    } else if (b == String.class && a.isAssignableFrom(sender.createCommandSourceStack().getClass())) {
                                                    ...
                                                    IslandCraftMod.LOGGER.info("Invoked dispatcher dispatch via reflection (matched {})", method);
                                                    dispInvoked = true;
                                                    break;
                                                } catch (Throwable exx) {
                                                    IslandCraftMod.LOGGER.warn("dispatcher method invocation threw: {}", exx.toString());
                                                }
                                            }
                                            if (!dispInvoked) {
                                                IslandCraftMod.LOGGER.warn("No suitable dispatcher.dispatch method found (inspected methods)");
                                                // As a robust fallback across mappings, try spawning a PrimedTnt entity
                                                try {
                                                    net.minecraft.commands.CommandSourceStack css = sender.createCommandSourceStack();
                                                    ServerLevel slevel = css.getLevel();
                                                    if (slevel != null) {
                                                        double dx = pkt.pos.getX() + 0.5D;
                                                        double dy = pkt.pos.getY() + 0.5D;
                                                        double dz = pkt.pos.getZ() + 0.5D;
                                                        PrimedTnt tnt = new PrimedTnt(slevel, dx, dy, dz, sender);
                                                        try {
                                                            tnt.setFuse(0);
                                                        } catch (Throwable __f) {
                                                            // some mappings may use a different setter name — ignore if absent
                                                        }
                                                        slevel.addFreshEntity(tnt);
                                                        IslandCraftMod.LOGGER.info("Spawned PrimedTnt entity at {} via fallback", pkt.pos);
                                                    } else {
                                                        IslandCraftMod.LOGGER.warn("Cannot spawn PrimedTnt: server level not available");
                                                    }
                                        ...
                                        IslandCraftMod.LOGGER.warn("getDispatcher() not available on commands object: {}", nsme2.toString());
                                    }
                                }
                            } catch (Throwable invEx) {
                                IslandCraftMod.LOGGER.warn("Failed to lookup/invoke command methods via reflection: {}", invEx.toString());
                            }
                        } else {
                            IslandCraftMod.LOGGER.warn("getCommands() returned null on server instance");
                        }
                    } catch (Throwable ex) {
                        IslandCraftMod.LOGGER.warn("Failed to access server commands via reflection: {}", ex.toString());
                    }
                } else {
